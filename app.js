...
function processNuevosData(data) {
    ...
    // Extract unique months, cycles, and vendors
    const uniqueMonths = new Set();
    const uniqueCycles = new Set();
    const uniqueVendors = new Set();

    data.forEach(row => {
        // Normal Months — use Marca temporal (Supabase) or Fecha de compra (CSV)
    ...
    data.forEach(row => {
        ...
        // Handle DD/MM/YYYY
            ...
            if (parts.length >= 2) {
                const m = parts[1].padStart(2, '0');
                const yRaw = parts.length === 3 ? parts[2] : '2025';
                const y = yRaw.split(' ')[0].trim(); // Remove time if present
        ...
        // Filter out if a specific month is selected
        if (STATE.selectedMonth !== 'all' && monthKey !== STATE.selectedMonth) {
            return;
        }
...
function updatePublicityUI() {
    ...
    // Filter leads by pub month filter
    ...
    // Populate month dropdown for pub view
    const monthSet = new Set();
    leads.forEach(l => {
        const dt = parseSheetDate(l['Fecha']);
        if (dt) monthSet.add(`${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(2, '0')}`);
    });
    ...
    if (pubSelect) {
        ...
        [...monthSet].sort().reverse().forEach(mk => {
            const [y, m] = mk.split('-');
            const label = new Date(y, parseInt(m) - 1, 1).toLocaleDateString('es-ES', { month: 'long', year: 'numeric' });
